...
function cv_fit2(y, evalpts, ptr, brks, istemporal, lambdas, cvsets, models)
    ...
    @threads for k in 1:nlambdas
        ...
        for i in 1:nsplits
            ...
            for j in cvsets[i]
                ...
                for yᵢ in test_eval
                    nloglikelihood += - eval_logprob(root, yᵢ)
                end
            end
    ...
    _, betas = fit2(y, ptr, brks, λ1, λ2, η1, η2, istemporal)

    lls = zeros(num_nodes)
    @threads for i in 1:num_nodes
        beta = betas[:, i]
        root = make_tree_from_bfs(lows, mids, ups, beta)
        ll = mean([eval_logprob(root, yᵢ) for yᵢ in samples[i]])
        lls[i] = ll
    end
...
for i in 1:30, j in 1:30
    ...
    res = [exp(eval_logprob(root, x)) for x in splitvals]
    fitprobs[i, j, :] = res
    fitdens[i, j, :] = (res ./ δx)
end
...
# npzwrite("simulations/benchmaks/example_task.npz", tosave)
...
CSV.write("benchmarks-results-spt.csv", df)
