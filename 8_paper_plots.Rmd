...
global_dens = chaindf %>%
  group_by(evalpt) %>%
  summarize(prob=exp(mean(log(prob))), .groups="drop")
```
...
src = src %>%
  ...
  filter(site_num %in% c(1, 5, 3, 4, 6), time_num %in% c(1, 35))
...
p = ggplot(src) +
  ...
  geom_text(
    ...
    data=filter(countlabs, site_num  %in% c(1, 5, 3, 4, 6), time_num %in% c(1, 35)),
    size=3,
    show.legend=FALSE
  ) +
...
chaindf %>% 
  ...
  summarize(q05=quantile(prob, 0.05), q95=quantile(prob, 0.95))
```
```{r}
src %>% 
  ungroup() %>% 
...
for (tplevel in tplevels) {
  ...
  min_val = round(min(tpmap$value), 2) - 0.01
  max_val = round(max(tpmap$value), 2) + 0.01
  ...
  for (t in times) {
    ...
    g = ggmap(austinmap, darken = c(0.3, "white")) +
      ...
      scale_fill_viridis(limits=c(min_val, max_val), n.breaks=10) +
...
max_val = 0.0701 # round(max(iqr21$value), 3) + 0.001
